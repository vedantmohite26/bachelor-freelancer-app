...
# Use AsyncInferenceClient for better concurrent performance
...
def get_cache_key(request: FinanceRequest):
    """Generate a collision-resistant MD5 hash for the request payload."""
    payload = f"{request.message}|{request.expenses}|{request.history}|{request.app_data}"
    return hashlib.md5(payload.encode()).hexdigest()

@app.post("/finance-ai")
async def finance_ai(request: FinanceRequest):
    print(f"Received request: {request.message}", flush=True)
    print(f"App Data: {request.app_data}", flush=True)  # Debug print
    ...
    if not HF_API_KEY:
        mock_response = "Listen, I'm currently in 'offline mode' because my brain (API key) is missing. But I'm still watching your ₹! 😉"
