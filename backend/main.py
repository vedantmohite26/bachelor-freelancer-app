...
# Use AsyncInferenceClient for better concurrency and non-blocking I/O
...
async def finance_ai(request: FinanceRequest):
    ...
    # Normalize expenses by truncating timestamp to hour precision to increase cache hits
    # Pattern matches "System Date/Time: YYYY-MM-DD HH:MM"
    ...
    cache_input = f"{request.message}|{normalized_expenses}|{str(request.history)}|{request.app_data}"
    ...
    # If no API key, return mock response for development (also cached)
    if not HF_API_KEY:
        mock_response = "I am a mock response because no HF_API_KEY is provided. I'm still cached though! ⚡"
    ...
    try:
        # Await the async chat completion
