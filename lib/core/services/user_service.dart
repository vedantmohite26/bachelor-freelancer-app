...
class UserService {
  ...
  // In-memory cache for user profiles to avoid redundant network requests.
  // We store the Future itself so multiple callers can wait on the same request.
  ...
  Future<Map<String, dynamic>?> getUserProfile(String userId) async {
    ...
    final future = _firestore.collection('users').doc(userId).get().then((doc) {
      ...
      // Remove from cache if request fails so next attempt can retry
      _userCache.remove(userId);
      throw error;
    });
