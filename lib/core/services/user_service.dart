...
class UserService {
  ...
  // In-memory cache for user profile futures to prevent redundant network requests
  // and handle "cache stampedes" by sharing the same future among concurrent callers.
  ...
  // Get user profile with in-memory caching (O(1) for cached hits)
  Future<Map<String, dynamic>?> getUserProfile(String userId) async {
    if (userId.isEmpty) return null;

    // Check if we have a pending or completed future in cache
    if (_profileCache.containsKey(userId)) {
      return _profileCache[userId];
    }

    // Create a new future for the fetch operation
    final Future<Map<String, dynamic>?> profileFuture = _firestore
        .collection('users')
        .doc(userId)
        .get()
        .then((doc) {
        ...
        .catchError((error) {
          // If fetch fails, remove from cache so it can be retried
          _profileCache.remove(userId);
          throw error;
        });

    // Store the future in cache before returning
    _profileCache[userId] = profileFuture;
