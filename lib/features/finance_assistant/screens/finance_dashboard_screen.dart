...
class _FinanceDashboardScreenState extends State<FinanceDashboardScreen> {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<List<FinanceTransaction>>(
        ...
        builder: (context, snapshot) {
          ...
          final metrics = _DashboardMetrics.compute(transactions);
  ...
  Widget _buildDailySpendingTrend(_DashboardMetrics metrics) {
    final now = DateTime.now();
    final List<double> dailySpending = metrics.dailySpendingLast7Days;
...
class _DashboardMetrics {
  ...
  final List<double> dailySpendingLast7Days;
  ...
  _DashboardMetrics({
    ...
    required this.dailySpendingLast7Days,
    required this.categoryTotals,
  });

  static _DashboardMetrics compute(List<FinanceTransaction> transactions) {
    final now = DateTime.now();
    double spentThisMonth = 0;
    double spentToday = 0;
    double incomeThisMonth = 0;
    final List<double> dailySpendingLast7Days = List.filled(7, 0.0);
    final Map<String, double> categoryTotals = {};

    final last7DaysDates = List.generate(7, (index) {
    ...
    for (final tx in transactions) {
      ...
      if (tx.date.year == now.year && tx.date.month == now.month) {
        ...
        } else if (tx.type == TransactionType.expense && !tx.isRecurring) {
      ...
      for (int i = 0; i < 7; i++) {
        if (txDate == last7DaysDates[i]) {
          if (tx.type == TransactionType.expense && !tx.isRecurring) {
            dailySpendingLast7Days[i] += tx.amount;
          }
          break;
        }
      }
    ...
    return _DashboardMetrics(
      ...
      dailySpendingLast7Days: dailySpendingLast7Days,
