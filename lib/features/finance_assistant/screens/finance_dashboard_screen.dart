...
class _FinanceDashboardScreenState extends State<FinanceDashboardScreen> {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<List<FinanceTransaction>>(
        ...
        builder: (context, snapshot) {
          ...
          final metrics = _DashboardMetrics.calculate(transactions, _selectedDate);
  ...
  Widget _buildDailySpendingTrend(_DashboardMetrics metrics) {
    final now = DateTime.now();
    final dailySpending = metrics.dailySpendingTrend;
  ...
  Widget _buildTopCategories(_DashboardMetrics metrics) {
    final categories = metrics.categoryTotals;
  ...
  Widget _buildDailyExplorer(_DashboardMetrics metrics) {
    return Column(
      ...
      children: [
        ...
        SizedBox(height: 8.h),
        _buildDateScroller(),
        _buildDailyTransactions(metrics.dailyTransactions),
      ],
    );
  }
  ...
  Widget _buildDailyTransactions(List<FinanceTransaction> dailyTx) {
    if (dailyTx.isEmpty) {
      return Center(
        child: Padding(
...
class _DashboardMetrics {
  ...
  final List<FinanceTransaction> dailyTransactions;
  ...
  factory _DashboardMetrics.calculate(
    ...
    for (final tx in transactions) {
      ...
      // Today's metrics
      if (isExpense && !tx.isRecurring && txDateOnly == todayDate) {
        spentToday += tx.amount;
      }

      // 7-day trend
      if (isExpense && !tx.isRecurring) {
        final difference = todayDate.difference(txDateOnly).inDays;
      ...
      // Daily explorer transactions
      if (txDateOnly == selDate) {
