          ...
          Expanded(
            child: StreamBuilder<List<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                ...
                // ⚡ Bolt: Single-pass distance filtering & pre-computation.
                // Calculating distance with Geolocator.distanceBetween in ListView itemBuilder
                // causes high CPU load during scrolling (O(N) trig calls on every frame render).
                // Pre-calculating distance and formatting display strings here reduces redundant
                // distance calculations to once per stream snapshot update.
                final List<Map<String, dynamic>> jobs = [];
                if (_currentPosition != null) {
                  for (final job in allJobs) {
                    if (job['latitude'] == null || job['longitude'] == null) {
                      job['_distanceDisplay'] = "...";
                      jobs.add(job);
                      continue;
                    }
                    try {
                      ...
                      if (distMeters <= _maxRadiusKm * 1000) {
                        job['_distanceDisplay'] = distMeters < 1000
                      ...
                      Expanded(
                        child: ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final timeDisplay = job['time'] ?? "Flexible";
                            return JobCard(
