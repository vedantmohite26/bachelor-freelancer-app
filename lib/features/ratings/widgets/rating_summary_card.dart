...
class _RatingSummaryCardState extends State<RatingSummaryCard> {
  Future<Map<int, int>>? _ratingDistributionFuture;
  ...
  void didUpdateWidget(RatingSummaryCard oldWidget) {
    ...
    // Refresh distribution if helperId changes OR if reviewCount increases (new review added)
    if (oldWidget.helperId != widget.helperId ||
      ...
      if (widget.reviewCount > 0) {
        ...
        setState(() {
          _ratingDistributionFuture = null;
        });
      }
    }
  }
  ...
  void _loadDistribution() {
    // Cache the future to prevent redundant Firestore calls on every rebuild
    _ratingDistributionFuture = Provider.of<RatingService>(
      context,
      listen: false,
            ...
            children: [
              ...
              Expanded(
                child: FutureBuilder<Map<int, int>>(
                  future: _ratingDistributionFuture,
